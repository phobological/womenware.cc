BeforeLoad = tick()
print("womenware.cc on top if not you like dick")
getgenv().values = {}       
local library = {}      
...
local cfglocation = "womenwarecccfg/"      
makefolder("womenwarecccfg")      
...
local MainUIColor = COL3RGB(255, 165, 0)    
...
local gui = library:New("Womenware.cc 0.6.9")      
		...
		pcall(function()
			...
			for i,v in pairs(debug.getupvalues(Client.setcharacter)) do
				if type(v) == "userdata" and v.ClassName == "AnimationTrack" and v.Name == "Idle" then
					CrouchAnim = v																																																																																																																																																																																																																																																																																																																																																						-- this_was_pasted_from_mexicanhook. damn fucking right it was, so to basedproperty on github for the sick ass base
				end
			end
...
chat:Element("Toggle", "chat spam", nil, function(tbl)      
	if tbl.Toggle then      
		while values.misc.chat["chat spam"].Toggle do      
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "standard" and "hi" or "Womenware.cc going chris brown", false, "Innocent", false, true)      
...
CreateHitElement("Setting up the kitchen...",Color3.new(1,1,0))
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)      
	...
	if values.misc.chat["kill say"].Toggle then      
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "" and values.misc.chat["message"].Text or "You got beat harder then chris browns wife with womenware.cc!", false, "Innocent", false, true)      
	end      
end)      
...
ChatScript.createNewMessage("Womenware.cc","The kitchen has been set up...",MainUIColor,Color3.new(1,1,1),0.01,nil)
ChatScript.moveOldMessages()
ChatScript.createNewMessage("Womenware.cc","0.6.9",MainUIColor,Color3.new(1,1,1),0.01,nil)